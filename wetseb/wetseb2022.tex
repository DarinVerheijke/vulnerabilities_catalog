...
Table~\ref{tab:transfer} show the Ether exchange and guard methods considering only the 9,176 contracts that contains at least one transfer function.  The percentage of contracts when looking at only contract with transfer is similar (with a 1-2\% difference) from the ones considering all contracts.  This is different from the contracts with call where the guards percentage increases by a noticeable amount for Assert and Revert. 
