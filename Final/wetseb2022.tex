...
\todo[inline]{Please, also change the figures in this version to have labels for the y and x-axis.}
...
We found a total of 1,190 reentrancy vulnerabilities in the 13,443 contracts that contained a call function. 
\todo[inline]{How many contracts were flagged with reentrancy? The percentage of contracts with reentrancy is more important than the average reentrancy vulnerabilities.}
...
The reentrancy vulnerability is thus still prevalent in contracts using a call value function up until this day.\todo[inline]{Is it? 1,190 is not even 10\% of the 13K contracts, and that is assuming you only have 1 reentrancy per contract. I would rephrase that by saying it is still occurring but not as common.}

We contrasted the reentrancy numbers for contracts with a call against contracts without a call function. Even without call, a contract may still face reentrancy issues by using functions from other contracts (which would transfer the control to the called contract). Only 231 had a reentrancy vulnerability which was significantly less then the contracts with a call function, on average we had about 0.05 reentrancy vulnerabilities per contract that did not contain a call function. 
\todo[inline]{231 out of how many contracts? And 231 is the number of reentrancies detected or the number of contracts with reentrancy? Are you sure it is 0.05? To me 231 /  13K = 0.01}
...
A reentrancy check on the contracts containing the unsafe call functionality showed that reentrancy vulnerabilities are still present in contracts even to this date.  \todo[inline]{Revise this after the Reentrancy Results is updated/revised.}
